//! Responsible for the syntax analysis stage of the Pugspeak compiler.
//!
//! This stage creates a hierarchical representation of your Pugspeak programs,
...
function PugspeakClassParser(lexer, builder) constructor {
    ...
    /// Updates the parser by parsing a simple Pugspeak expression from the
    ...
    static update = function () {
        if (lexer.peek() == PugspeakToken.EOF) {
            if (!finalised) {
                asg.popFunction();
                finalised = true;
            }
    ...
    static __parseStatement = function () {
        ...
        if (peeked == PugspeakToken.SEMICOLON) {
            lexer.next();
            return;
        } else if (peeked == PugspeakToken.LET) {
            lexer.next();
            if (lexer.next() != PugspeakToken.IDENT) {
                __ex("expected identifier after 'let' keyword");
            }
            var localName = lexer.getValue();
            var location = lexer.getLocation();
            var valueTerm;
            if (lexer.peek() == PugspeakToken.ASSIGN) {
                lexer.next();
    ...
    static __parseExpression = function () {
        var peeked = lexer.peek();
        if (peeked == PugspeakToken.RETURN) {
            lexer.next();
            peeked = lexer.peek();
            var value;
            if (
                peeked == PugspeakToken.SEMICOLON ||
                peeked == PugspeakToken.BRACE_RIGHT
            ) {
        ...
        } else if (peeked == PugspeakToken.CONTINUE) {
            lexer.next();
            return asg.createContinue(lexer.getLocation());
        } else if (peeked == PugspeakToken.BREAK) {
            lexer.next();
            peeked = lexer.peek();
            var value;
            if (
        ...
        } else if (peeked == PugspeakToken.DO) {
            lexer.next();
            asg.pushBlock(true);
            __parseStatements("do");
            return asg.popBlock();
        } else if (peeked == PugspeakToken.IF) {
            lexer.next();
            ...
            var ifFalse;
            if (lexer.peek() == PugspeakToken.ELSE) {
                lexer.next();
                asg.pushBlock();
                if (lexer.peek() == PugspeakToken.IF) {
                    // for `else if` support
        ...
        } else if (peeked == PugspeakToken.WHILE) {
            lexer.next();
            var condition = __parseCondition();
            asg.pushBlock();
        ...
        } else if (peeked == PugspeakToken.USE) {
            lexer.next();
            var condition = __parseCondition();
            asg.pushBlock();
        ...
        } else if (peeked == PugspeakToken.FUN) {
            lexer.next();
            asg.pushFunction();
            if (lexer.peek() != PugspeakToken.BRACE_LEFT) {
                if (lexer.next() != PugspeakToken.PAREN_LEFT) {
                    __ex("expected opening '(' after 'fun' keyword");
                }
                while (__isNot(PugspeakToken.PAREN_RIGHT)) {
                    if (lexer.next() != PugspeakToken.IDENT) {
                        __ex("expected identifier in function arguments");
                    }
                    asg.allocArg(lexer.getValue(), lexer.getLocation());
                    if (lexer.peek() == PugspeakToken.COMMA) {
                        lexer.next();
                    }
                }
                if (lexer.next() != PugspeakToken.PAREN_RIGHT) {
    ...
    static __parseStatements = function (keyword) {
        if (lexer.next() != PugspeakToken.BRACE_LEFT) {
            __ex("expected opening '{' at the start of '", keyword, "' block");
        }
        while (__isNot(PugspeakToken.BRACE_RIGHT)) {
            __parseStatement();
        }
        if (lexer.next() != PugspeakToken.BRACE_RIGHT) {
    ...
    static __parseAssign = function () {
        ...
        if (
            ...
            peeked == PugspeakToken.ASSIGN_MULTIPLY ||
            peeked == PugspeakToken.ASSIGN_DIVIDE ||
            peeked == PugspeakToken.ASSIGN_SUBTRACT ||
            peeked == PugspeakToken.ASSIGN_PLUS
        ) {
            lexer.next();
            var assignType = __pugspeak_operator_assign_from_token(peeked);
    ...
    static __parseOpBitwise = function () {
        ...
        while (true) {
            var peeked = lexer.peek();
            if (
                peeked == PugspeakToken.BITWISE_AND ||
                peeked == PugspeakToken.BITWISE_XOR ||
                peeked == PugspeakToken.BITWISE_OR
            ) {
                lexer.next();
    ...
    static __parseOpBitwiseShift = function () {
        ...
        while (true) {
            var peeked = lexer.peek();
            if (
                peeked == PugspeakToken.SHIFT_LEFT ||
                peeked == PugspeakToken.SHIFT_RIGHT
            ) {
                lexer.next();
                var op = __pugspeak_operator_from_token(peeked);
                var lhs = result;
    ...
    static __parseOpEquality = function () {
        ...
        while (true) {
            var peeked = lexer.peek();
            if (
                peeked == PugspeakToken.EQUAL ||
                peeked == PugspeakToken.NOT_EQUAL
            ) {
                lexer.next();
                var op = __pugspeak_operator_from_token(peeked);
                var lhs = result;
    ...
    static __parseOpRelational = function () {
        ...
        while (true) {
            var peeked = lexer.peek();
            if (
                peeked == PugspeakToken.LESS ||
                peeked == PugspeakToken.LESS_EQUAL ||
                peeked == PugspeakToken.GREATER ||
                peeked == PugspeakToken.GREATER_EQUAL
            ) {
                lexer.next();
                var op = __pugspeak_operator_from_token(peeked);
                var lhs = result;
    ...
    static __parseOpAdd = function () {
        ...
        while (true) {
            var peeked = lexer.peek();
            if (
                peeked == PugspeakToken.PLUS ||
                peeked == PugspeakToken.SUBTRACT
            ) {
                lexer.next();
                var op = __pugspeak_operator_from_token(peeked);
                var lhs = result;
    ...
    static __parseOpMultiply = function () {
        ...
        while (true) {
            var peeked = lexer.peek();
            if (
                peeked == PugspeakToken.MULTIPLY ||
                peeked == PugspeakToken.DIVIDE ||
                peeked == PugspeakToken.DIVIDE_INT ||
                peeked == PugspeakToken.REMAINDER
            ) {
                lexer.next();
                var op = __pugspeak_operator_from_token(peeked);
                var lhs = result;
    ...
    static __parseIndex = function () {
        ...
        while (true) {
            var peeked = lexer.peek();
            if (peeked == PugspeakToken.PAREN_LEFT) {
    ...
    static __parseGrouping = function () {
        ...
        if (peeked == PugspeakToken.PAREN_LEFT) {
            ...
            lexer.next();
            var values = [];
            while (__isNot(PugspeakToken.BOX_RIGHT)) {
        ...
        } else if (peeked == PugspeakToken.BRACE_LEFT) {
            lexer.next();
            var values = [];
            while (__isNot(PugspeakToken.BRACE_RIGHT)) {
                var key;
                var keyToken = lexer.next();
                if (keyToken == PugspeakToken.BOX_LEFT) {
                    ...
                    keyToken == PugspeakToken.IDENT ||
                    keyToken == PugspeakToken.VALUE
                ) {
                ...
                var value;
                if (lexer.peek() == PugspeakToken.COLON) {
                    lexer.next();
                    value = __parseExpression();
                } else if (keyToken == PugspeakToken.IDENT) {
    ...
    static __ex = function () {
        var dbg = __pugspeak_location_show(lexer.getLocation()) + " when parsing";
        if (argument_count < 1) {
            __pugspeak_error(dbg);
        } else {
            var msg = "";
            for (var i = 0; i < argument_count; i += 1) {
                msg += __pugspeak_string(argument[i]);
            }
            __pugspeak_error(dbg, " -- ", msg, ", got ", __tokenDebug());
        }
    };
    ...
    static __tokenDebug = function () {
        var peeked = lexer.peek();
        if (peeked == PugspeakToken.EOF) {
            return "end of file";
        } else if (peeked == PugspeakToken.SEMICOLON) {
            return "line break ';'";
        }
    ...
    /// @ignore
    ///
    /// @param {Enum.PugspeakToken} expect
    /// @return {Bool}
    static __isNot = function (expect) {
        var peeked = lexer.peek();
        return peeked != expect && peeked != PugspeakToken.EOF;
    };
