...
/// @param {Function} func
/// @return {String}
function __pugspeak_infer_function_name(func) {
    if (is_method(func)) {
        var name = func[$ "name"];
        if (is_string(name)) {
            return name;
...
function PugspeakForeignInterface() constructor {
    ...
    ///
    /// @param {String} name
    ///   The name of the symbol as it appears in Pugspeak.
    ///
    /// @return {Any}
    ...
    ///
    /// @param {String} ban
    ///   The symbol to ban the usage of from within Pugspeak.
    static addBanList = function () {
        var banList_ = banList;
        for (var i = 0; i < argument_count; i += 1) {
            var ban = argument[i];
            if (PUGSPEAK_DEBUG_MODE) {
                __pugspeak_check_arg("ban", ban, is_string);
            }
            banList_[$ ban] = true;
        }
    };
    ...
    ///
    /// @param {String} pardon
    ///   The symbol to pardon the usage of from within Pugspeak.
    static addPardonList = function () {
        var banList_ = banList;
        for (var i = 0; i < argument_count; i += 1) {
            var pardon = argument[i];
            if (PUGSPEAK_DEBUG_MODE) {
                __pugspeak_check_arg("pardon", pardon, is_string);
            }
    ...
    static exposeConstant = function () {
        for (var i = 0; i < argument_count; i += 2) {
            ...
            if (PUGSPEAK_DEBUG_MODE) {
                ...
                //__pugspeak_check_arg_not("value", value, __pugspeak_is_callable);
            }
            database[$ name] = value;
        }
    };
    ...
    static exposeFunction = function () {
        for (var i = 0; i < argument_count; i += 2) {
            ...
            if (PUGSPEAK_DEBUG_MODE) {
                __pugspeak_check_arg("name", name, is_string);
                //__pugspeak_check_arg("func", func, __pugspeak_is_callable);
            }
    ...
    static exposeFunctionByName = function () {
        for (var i = 0; i < argument_count; i += 1) {
            ...
            var name = __pugspeak_infer_function_name(func);
            func = is_method(func) ? method_get_index(func) : func;
            database[$ name] = method(undefined, func);
        }
    };
    ...
    static exposeFunctionByPrefix = function () {
        for (var i = 0; i < argument_count; i += 1) {
            ...
            if (PUGSPEAK_DEBUG_MODE) {
                __pugspeak_check_arg("namespace", namespace, is_string);
            }
    ...
    ///
    /// @param {String} name
    ///   The name of the method as it will appear in Pugspeak.
    ///
    /// @param {Function} func
    ...
    ///
    /// @param {String} name
    ///   The name of the GM asset that you wish to expose to Pugspeak.
    ...
    ///
    /// @param {Any} tag
    ///   The name of a tag, or array of tags, of assets to expose to Pugspeak.
...
/// The number of microseconds before a Pugspeak program times out. The
/// default is 1 second.
#macro PUGSPEAK_TIMEOUT 1000

/// @ignore
///
/// @param {Real} t
function __pugspeak_timeout_check(t) {
    gml_pragma("forceinline");
    if (current_time - t > PUGSPEAK_TIMEOUT) {
        __pugspeak_error(
            "process exceeded allowed time of ", PUGSPEAK_TIMEOUT, " ms"
        );
    }
}
...
function PugspeakGMLCompiler(asg, interface=undefined) constructor {
    ...
    static update = function () {
        if (PUGSPEAK_DEBUG_MODE && finalised) {
            __pugspeak_error(
    ...
    static __compileFunctions = function (entryPoints) {
        ...
        for (var i = 0; i < entryCount; i += 1) {
            ...
            if (PUGSPEAK_DEBUG_MODE) {
                __pugspeak_check_arg("entry", entry, is_numeric);
            }
            exprs[@ i] = __compileFunction(functions_[entry]);
        }
        var rootCall = __emitBlock(exprs);
        __setupPugspeakFunctionMethods(rootCall);
        return rootCall;
    };
    ...
    static __compileFunction = function (func) {
        if (PUGSPEAK_DEBUG_MODE) {
            __pugspeak_check_arg_struct("func", func,
                "localCount", is_numeric,
                "argCount", is_numeric,
                "root", undefined
            );
            __pugspeak_check_arg_struct("func.root", func.root,
                "type", is_numeric
            );
        }
        var ctx = {
        ...
        if (__pugspeak_term_is_pure(func.root.type)) {
            // if there's absolutely no way this function could misbehave,
            // use the fast path
            return ctx.program;
        }
        var _method = method(ctx, __pugspeak_function__);
    ...
    static __compileValue = function (ctx, term) {
        ...
        return method({ value : term.value }, __pugspeak_expr_value__);
    };
    ...
    static __emitBlock = function (exprs) {
        ...
        if (exprCount == 1) {
            ...
            return method({
                _1st : exprs[0],
                _2nd : exprs[1],
            }, __pugspeak_expr_block_2__);
        } else if (exprCount == 3) {
            return method({
                _1st : exprs[0],
                _2nd : exprs[1],
                _3rd : exprs[2],
            }, __pugspeak_expr_block_3__);
        } else if (exprCount == 4) {
            return method({
                _1st : exprs[0],
                _2nd : exprs[1],
    ...
    static __compileIf = function (ctx, term) {
        ...
        if (term.ifFalse == undefined) {
            ...
            }, __pugspeak_expr_if__);
        } else {
            return method({
                condition : __compileTerm(ctx, term.condition),
                ifTrue : __compileTerm(ctx, term.ifTrue),
                ifFalse : __compileTerm(ctx, term.ifFalse),
            }, __pugspeak_expr_if_else__);
        }
    };
    ...
    static __compileContinue = function (ctx, term) {
        return method(undefined, __pugspeak_expr_continue__);
    };
    ...
    static __compileOpUnary = function (ctx, term) {
        ...
        return method({
            op : __pugspeak_operator_get_unary(term.operator),
            value : __compileTerm(ctx, term.value),
        }, __pugspeak_expr_op_1__);
    };
    ...
    static __compileOpBinary = function (ctx, term) {
        ...
        return method({
            op : __pugspeak_operator_get_binary(term.operator),
            lhs : __compileTerm(ctx, term.lhs),
            rhs : __compileTerm(ctx, term.rhs),
        }, __pugspeak_expr_op_2__);
    };
    ...
    static __compileCall = function (ctx, term) {
        ...
        if (term.callee.type == PugspeakTerm.INDEX) {
            if (PUGSPEAK_DEBUG_MODE) {
                __pugspeak_check_arg_struct("term.callee", term.callee,
    ...
    static __compileSet = function (ctx, term) {
        if (PUGSPEAK_DEBUG_MODE) {
            ...
            __pugspeak_check_arg_struct("term.target", term.target,
                "type", is_numeric
            );
        }
        ...
        if (targetType == PugspeakTerm.INDEX) {
            if (PUGSPEAK_DEBUG_MODE) {
            ...
            if (__exists(name)) {
                ...
                __pugspeak_error(
                    __pugspeak_location_show(target.dbg),
    ...
    static __compileFunctionExpr = function (ctx, term) {
        ...
        __setupPugspeakFunctionMethods(funcExpr);
        return method({
            value : funcExpr,
        }, __pugspeak_expr_value__);
    };
    ...
    static __compileTerm = function (ctx, term) {
        ...
        if (PUGSPEAK_DEBUG_MODE && prod == undefined) {
    ...
    static __productionLookup = (function () {
        var db = array_create(PugspeakTerm.__SIZE__, undefined);
        db[@ PugspeakTerm.VALUE] = __compileValue;
        db[@ PugspeakTerm.ARRAY] = __compileArray;
        db[@ PugspeakTerm.STRUCT] = __compileStruct;
        db[@ PugspeakTerm.BLOCK] = __compileBlock;
        db[@ PugspeakTerm.IF] = __compileIf;
        db[@ PugspeakTerm.WHILE] = __compileWhile;
        db[@ PugspeakTerm.USE] = __compileUse;
        db[@ PugspeakTerm.RETURN] = __compileReturn;
        db[@ PugspeakTerm.BREAK] = __compileBreak;
        db[@ PugspeakTerm.CONTINUE] = __compileContinue;
    ...
    static __assignLookupIndex = (function () {
        ...
        db[@ PugspeakAssign.VANILLA] = __pugspeak_expr_index_set__;
        db[@ PugspeakAssign.MULTIPLY] = __pugspeak_expr_index_set_mult__;
    ...
    static __assignLookupLocal = (function () {
        var db = array_create(PugspeakAssign.__SIZE__, undefined);
        db[@ PugspeakAssign.VANILLA] = __pugspeak_expr_local_set__;
        db[@ PugspeakAssign.MULTIPLY] = __pugspeak_expr_local_set_mult__;
        db[@ PugspeakAssign.DIVIDE] = __pugspeak_expr_local_set_div__;
        db[@ PugspeakAssign.SUBTRACT] = __pugspeak_expr_local_set_sub__;
        db[@ PugspeakAssign.PLUS] = __pugspeak_expr_local_set_plus__;
        return db;
    })();
    ...
    static __dbgTerm = function (term, msg="is invalid in this context") {
        ...
        var terminalName = __pugspeak_term_get_terminal(term);
        return "runtime error " + __pugspeak_location_show_ext(term.dbg,
...
function __pugspeak_function__() {
    ...
    if (isRecursing) {
        ...
        __pugspeak_timeout_check(callTime);
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_value__() {
    return value;
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_array__() {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_struct__() {
    var obj = { };
    var i = 0;
    var values_ = values;
    var n_ = n;
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_block__() {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_block_2__() {
    _1st();
    return _2nd();
}
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_block_3__() {
    _1st();
    _2nd();
    return _3rd();
}
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_block_4__() {
    _1st();
    _2nd();
    _3rd();
    return _4th();
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_block_5__() {
    _1st();
    _2nd();
    _3rd();
    _4th();
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_if__() {
    return condition() ? ifTrue() : undefined;
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_if_else__() {
    return condition() ? ifTrue() : ifFalse();
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_and__() {
    return eager() && lazy();
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_or__() {
    return eager() || lazy();
}
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_use__() {
    var body_ = body;
    var open = condition();
    if (!is_method(open)) {
        __pugspeak_error_got(dbgError, open);
    }
    var close = open();
    if (!is_method(close)) {
        __pugspeak_error_got(dbgError, close);
    }
    var result;
    try {
        body_();
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_while_simple__() {
    var callTime = ctx.callTime;
    var condition_ = condition;
    var body_ = body;
    while (condition_()) {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_op_1__() {
    var value_ = value();
    return op(value_);
}
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_op_2__() {
    var lhs_ = lhs();
    var rhs_ = rhs();
    return op(lhs_, rhs_);
}
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_call_method__() {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_call__() {
    var callee_ = callee();
    if (!is_method(callee_)) {
        __pugspeak_error_got(dbgError, callee_);
    }
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_index_set__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_index_set_mult__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_index_set_div__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_index_set_sub__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_index_set_plus__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
/// @ignore
/// @return {Any}
function __pugspeak_expr_local_get__() {
    return locals[idx];
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_local_set__() {
    locals[@ idx] = value();
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_local_set_mult__() {
    locals[@ idx] *= value();
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_local_set_div__() {
    locals[@ idx] /= value();
}

/// @ignore
/// @return {Any}
function __pugspeak_expr_local_set_sub__() {
    locals[@ idx] -= value();
}
