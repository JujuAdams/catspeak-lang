//! Responsible for the lexical analysis stage of the Pugspeak compiler.

//# feather use syntax-errors

/// A token in Pugspeak is a series of characters with meaning, usually
/// separated by whitespace.
///
/// For example, these are all valid tokens:
///   - `if`   (is a `PugspeakToken.IF`)
///   - `else` (is a `PugspeakToken.ELSE`)
///   - `12.3` (is a `PugspeakToken.VALUE`)
///   - `+`    (is a `PugspeakToken.PLUS`)
///
...
/// @param {String} src
/// @return {Id.Buffer}
function __pugspeak_create_buffer_from_string(src) {
...
/// for syntax highlighting in a programming game which uses the Pugspeak
/// env.
///
...
function PugspeakClassLexer(
    ...
    if (PUGSPEAK_DEBUG_MODE) {
        ...
        __pugspeak_check_arg("offset", offset, is_numeric);
        __pugspeak_check_arg("size", size, is_numeric);
        if (keywords != undefined) {
            __pugspeak_check_arg("keywords", keywords, is_struct);
        }
    }
    self.buff = buff;
    ...
    self.lexemePos = pugspeak_location_create(self.row, self.column);
    ...
    static __nextUTF8Char = function () {
        ...
        if ((byte & 0xFC) == 0xFC) { // if ((byte & 0b11111100) == 0b11111100) {
            ...
            //__pugspeak_error("invalid UTF8 header codepoint '", byte, "'");
            return -1;
        }
        ...
        for (var i = codepointCount - 1; i >= 0; i -= 1) {
            ...
            buffOffset += 1;
            if ((byte & 0x80) == 0) { // if ((byte & 0b10000000) == 0) {
                //__pugspeak_error("invalid UTF8 continuation codepoint '", byte, "'");
                return -1;
            }
    ...
    static __clearLexeme = function () {
        ...
        lexemePos = pugspeak_location_create(self.row, self.column);
        lexeme = undefined;
        hasValue = false;
    };
    ...
    static __slice = function (start, end_) {
        ...
        if (clipEnd <= clipStart) {
            ...
            if (PUGSPEAK_DEBUG_MODE && clipEnd < clipStart) {
    ...
    ///
    /// @example
    ///   Prints the string content of the first [PugspeakToken] emitted by a
    ///   lexer.
    ///
    /// ```gml
    /// lexer.next();
    ...
    static __getKeyword = function (str) {
        ...
        if (PUGSPEAK_DEBUG_MODE && keyword != undefined) {
            __pugspeak_check_arg(
                    "keyword", keyword, __pugspeak_is_token, "PugspeakToken");
        }
        return keyword;
    };
    ...
    static getLocation = function () {
        return pugspeak_location_create(row, column);
    };

    /// Advances the lexer and returns the next type of [PugspeakToken]. This
    /// includes additional whitespace and control tokens, like:
    ///  - comments `--`            (`PugspeakToken.COMMENT`)
    ///
    /// To get the string content of the token, you should use the [getLexeme]
    /// method.
    ///
    /// @example
    ///   Iterates through all tokens of a buffer containing Pugspeak code,
    ...
    ///     show_debug_message(lexer.getLexeme());
    ///   }
    /// } until (token == PugspeakToken.EOF);
    /// ```
    ///
    /// @return {Enum.PugspeakToken}
    static nextWithWhitespace = function () {
        __clearLexeme();
        if (charNext == 0) {
            return PugspeakToken.EOF;
        }
        __advance();
        ...
        if (
            ...
            if (isRaw) {
                token = PugspeakToken.VALUE; // since `@` is an operator
                __advance();
            }
            ...
            value = value_;
            hasValue = true;
        } else if (__pugspeak_char_is_operator(charCurr_)) {
            // operators
            while (__pugspeak_char_is_operator(charNext)) {
                __advance();
            }
            var keyword = __getKeyword(getLexeme());
            if (keyword != undefined) {
                token = keyword;
                if (keyword == PugspeakToken.COMMENT) {
                    // consume the comment
            ...
            while (true) {
                ...
                if (
                    ...
                    __pugspeak_char_is_whitespace(charNext_)
                ) {
                    break;
                }
                __advance();
        ...
        } else if (token == PugspeakToken.IDENT) {
            // alphanumeric identifiers
            while (__pugspeak_char_is_alphanumeric(charNext)) {
                __advance();
            }
            ...
            while (true) {
                var charNext_ = charNext;
                if (__pugspeak_char_is_digit_hex(charNext_)) {
                    ds_stack_push(digitStack,
                            __pugspeak_char_hex_to_dec(charNext_));
                    __advance();
            ...
            while (true) {
                var charNext_ = charNext;
                if (__pugspeak_char_is_digit_binary(charNext_)) {
                    ds_stack_push(digitStack,
                            __pugspeak_char_binary_to_dec(charNext_));
                    __advance();
                } else if (charNext_ == ord("_")) {
                    __advance();
                } else {
            ...
            if (digitCount == 3) {
                ...
                // invalid
                token = PugspeakToken.OTHER;
            }
            ds_stack_destroy(digitStack);
            value = cR | (cG << 8) | (cB << 16) | (cA << 24);
            hasValue = true;
        } else if (token == PugspeakToken.VALUE) {
            // numeric literals
            var hasUnderscores = false;
            var hasDecimal = false;
            while (true) {
                var charNext_ = charNext;
                if (__pugspeak_char_is_digit(charNext_)) {
                    __advance();
                } else if (charNext_ == ord("_")) {
                    __advance();
                    hasUnderscores = true;
    ...
    /// Advances the lexer and returns the next [PugspeakToken], ingoring
...
/// @ignore
#macro __PUGSPEAK_CODEPAGE_SIZE 256
...
/// @ignore
function __pugspeak_char_is_digit(char) {
    gml_pragma("forceinline");
    return char >= ord("0") && char <= ord("9");
}

/// @ignore
function __pugspeak_char_is_digit_binary(char) {
    gml_pragma("forceinline");
    return char == ord("0") || char == ord("1");
}

/// @ignore
function __pugspeak_char_binary_to_dec(char) {
    gml_pragma("forceinline");
    return char == ord("0") ? 0 : 1;
}

/// @ignore
function __pugspeak_char_is_digit_hex(char) {
    gml_pragma("forceinline");
...
/// @ignore
function __pugspeak_char_hex_to_dec(char) {
...
/// @ignore
function __pugspeak_char_is_alphanumeric(char) {
    gml_pragma("forceinline");
...
/// @ignore
function __pugspeak_char_is_operator(char) {
    gml_pragma("forceinline");
...
/// @ignore
function __pugspeak_char_is_whitespace(char) {
    gml_pragma("forceinline");
    return char >= 0x09 && char <= 0x0D || char == 0x20 || char == 0x85;
}

/// @ignore
function __pugspeak_codepage_value(code) {
    gml_pragma("forceinline");
    return is_string(code) ? ord(code) : code;
}

/// @ignore
function __pugspeak_codepage_range(code, minCode, maxCode) {
    gml_pragma("forceinline");
    var codeVal = __pugspeak_codepage_value(code);
    var minVal = __pugspeak_codepage_value(minCode);
    var maxVal = __pugspeak_codepage_value(maxCode);
    return codeVal >= minVal && codeVal <= maxVal;
}

/// @ignore
function __pugspeak_codepage_set(code) {
    gml_pragma("forceinline");
    var codeVal = __pugspeak_codepage_value(code);
    for (var i = 1; i < argument_count; i += 1) {
        if (codeVal == __pugspeak_codepage_value(argument[i])) {
            return true;
        }
    }
    return false;
...
/// @ignore
function __pugspeak_init_lexer_codepage() {
    var page = array_create(__PUGSPEAK_CODEPAGE_SIZE, PugspeakToken.OTHER);
    for (var code = 0; code < __PUGSPEAK_CODEPAGE_SIZE; code += 1) {
        var tokenType;
        if (__pugspeak_codepage_set(code,
            ...
            tokenType = PugspeakToken.WHITESPACE;
        } else if (
            __pugspeak_codepage_range(code, "a", "z") ||
            __pugspeak_codepage_range(code, "A", "Z") ||
            __pugspeak_codepage_set(code, "_", "`") // identifier literals
        ) {
            tokenType = PugspeakToken.IDENT;
        } else if (
            __pugspeak_codepage_range(code, "0", "9") ||
            __pugspeak_codepage_set(code, "'") // character literals
        ) {
            tokenType = PugspeakToken.VALUE;
        } else if (__pugspeak_codepage_set(code, "\"")) {
            tokenType = PugspeakToken.VALUE;
        } else if (__pugspeak_codepage_set(code, "(")) {
            tokenType = PugspeakToken.PAREN_LEFT;
        } else if (__pugspeak_codepage_set(code, ")")) {
            tokenType = PugspeakToken.PAREN_RIGHT;
        } else if (__pugspeak_codepage_set(code, "[")) {
            tokenType = PugspeakToken.BOX_LEFT;
        } else if (__pugspeak_codepage_set(code, "]")) {
            tokenType = PugspeakToken.BOX_RIGHT;
        } else if (__pugspeak_codepage_set(code, "{")) {
            tokenType = PugspeakToken.BRACE_LEFT;
        } else if (__pugspeak_codepage_set(code, "}")) {
            tokenType = PugspeakToken.BRACE_RIGHT;
        } else if (__pugspeak_codepage_set(code, ",")) {
            tokenType = PugspeakToken.COMMA;
        } else {
            continue;
        }
        page[@ code] = tokenType;
...
///
/// @return {Struct}
function __pugspeak_keywords_create() {
    var keywords = { };
    keywords[$ "--"] = PugspeakToken.COMMENT;
    keywords[$ ";"] = PugspeakToken.SEMICOLON;
    keywords[$ ":"] = PugspeakToken.COLON;
    keywords[$ ","] = PugspeakToken.COMMA;
    keywords[$ "."] = PugspeakToken.DOT;
    keywords[$ "="] = PugspeakToken.ASSIGN;
    keywords[$ "*="] = PugspeakToken.ASSIGN_MULTIPLY;
    keywords[$ "/="] = PugspeakToken.ASSIGN_DIVIDE;
    keywords[$ "-="] = PugspeakToken.ASSIGN_SUBTRACT;
    keywords[$ "+="] = PugspeakToken.ASSIGN_PLUS;
    keywords[$ "%"] = PugspeakToken.REMAINDER;
    keywords[$ "*"] = PugspeakToken.MULTIPLY;
    keywords[$ "/"] = PugspeakToken.DIVIDE;
    keywords[$ "//"] = PugspeakToken.DIVIDE_INT;
    keywords[$ "-"] = PugspeakToken.SUBTRACT;
    keywords[$ "+"] = PugspeakToken.PLUS;
    keywords[$ "=="] = PugspeakToken.EQUAL;
    keywords[$ "!="] = PugspeakToken.NOT_EQUAL;
    keywords[$ ">"] = PugspeakToken.GREATER;
    keywords[$ ">="] = PugspeakToken.GREATER_EQUAL;
    keywords[$ "<"] = PugspeakToken.LESS;
    keywords[$ "<="] = PugspeakToken.LESS_EQUAL;
    keywords[$ "!"] = PugspeakToken.NOT;
    keywords[$ "~"] = PugspeakToken.BITWISE_NOT;
    keywords[$ ">>"] = PugspeakToken.SHIFT_RIGHT;
    keywords[$ "<<"] = PugspeakToken.SHIFT_LEFT;
    keywords[$ "&"] = PugspeakToken.BITWISE_AND;
    keywords[$ "^"] = PugspeakToken.BITWISE_XOR;
    keywords[$ "|"] = PugspeakToken.BITWISE_OR;
    keywords[$ "and"] = PugspeakToken.AND;
    keywords[$ "or"] = PugspeakToken.OR;
    keywords[$ "do"] = PugspeakToken.DO;
    keywords[$ "if"] = PugspeakToken.IF;
    keywords[$ "else"] = PugspeakToken.ELSE;
    keywords[$ "while"] = PugspeakToken.WHILE;
    keywords[$ "for"] = PugspeakToken.FOR;
    keywords[$ "loop"] = PugspeakToken.LOOP;
    keywords[$ "use"] = PugspeakToken.USE;
    keywords[$ "let"] = PugspeakToken.LET;
    keywords[$ "fun"] = PugspeakToken.FUN;
    keywords[$ "params"] = PugspeakToken.PARAMS;
...
function __pugspeak_keywords_rename(keywords, currentName, newName) {
...
///
/// @return {String}
function __pugspeak_keywords_find_name(keywords, token) {
    if (PUGSPEAK_DEBUG_MODE) {
        __pugspeak_check_arg("keywords", keywords, is_struct);
        __pugspeak_check_arg(
                "token", token, __pugspeak_is_token, "PugspeakToken");
    }
